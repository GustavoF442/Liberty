...
export default function ImovelDetalhesPage() {
    ...
    async function fetchData() {
      ...
      if (im.corretor_id) {
        const { data: corretorData } = await supabase
          .from("corretores")
          .select("*")
          .eq("id", im.corretor_id)
          .single();
        if (corretorData) setCorretor(corretorData as Corretor);
      }
  ...
  if (erro || !imovel) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/imoveis" className="text-verde hover:underline mt-4 inline-block">
          Voltar para imóveis
        </Link>
      </div>
    );
  ...
  const whatsappNumber = corretor?.telefone?.replace(/\D/g, "") || "5500000000000";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {/* Características — destaque visual */}
              ...
              {contatoEnviado ? (
                ...
                <form onSubmit={handleContatoSubmit} className="space-y-3">
                  <input
                    ...
                    className="w-full border border-areia-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-verde-light focus:border-verde-light outline-none bg-white"
                  />
                  <input
                    required
                    type="email"
                  ...
                  <textarea
                    ...
                    value={contatoMsg}
                    onChange={(e) => setContatoMsg(e.target.value)}
                    className="w-full border border-areia-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-verde-light focus:border-verde-light outline-none bg-white resize-y"
                  />
